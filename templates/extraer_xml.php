...
$invalidFiles = [];
$error = '';
$showInvalidFilesDownload = false;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['archivos_xml'])) {
    ...
    foreach ($_FILES['archivos_xml']['tmp_name'] as $key => $tmp) {
        ...
        if ($_FILES['archivos_xml']['error'][$key] !== UPLOAD_ERR_OK || $_FILES['archivos_xml']['size'][$key] <= 0) {
        ...
        if (move_uploaded_file($tmp, "$tempDir/$filename")) {
    ...
    if (!empty($uploadedFiles)) {
        ...
        if ($returnVar === 0 && file_exists($excelPath)) {
            ...
            $error = "Error al procesar XML. Detalle: " . implode("\n", $output);
        }
    } else {
...
<div class="alert alert-info"><i class="fas fa-info-circle"></i> Formatos soportados: CFDI 3.3 y 4.0</div>
</div>
