...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['archivos_xml'])) {
    ...
    if (!$error) {
        ...
        foreach ($_FILES['archivos_xml']['tmp_name'] as $key => $tmp) {
            $originalName = $_FILES['archivos_xml']['name'][$key] ?? '';
            $filename = $originalName ? basename($originalName) : "archivo_{$key}.xml";
            $fileError = $_FILES['archivos_xml']['error'][$key] ?? UPLOAD_ERR_NO_FILE;
            $fileSize = $_FILES['archivos_xml']['size'][$key] ?? 0;
...
<div class="form-group">
  <div class="file-upload">
    ...
    <label for="archivos_xml" class="file-upload-label">
      ...
      <span class="file-upload-title">Arrastra y suelta tus XML (CFDI)</span>
...
<div class="tool-output">
    ...
    <div class="tool-output-actions">
      ...
      <a href="extraer_xml.php" class="btn-secondary"><i class="fas fa-redo"></i> Subir más archivos</a>
    </div>
</div>
<?php endif; ?>
</section>
